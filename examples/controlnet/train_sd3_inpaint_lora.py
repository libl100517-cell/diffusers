...
import diffusers
from diffusers import AutoencoderKL, FlowMatchEulerDiscreteScheduler, SD3ControlNetModel, SD3Transformer2DModel
...
class CrackInpaintDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        conditioning_pixel_values = self.image_transforms(masked_image)
        conditioning_mask = self.mask_transforms(train_mask)
...
def collate_fn(examples):
    ...
    conditioning_mask = torch.stack([example["conditioning_mask"] for example in examples])
...
def main(args):
    ...
    if controlnet.config.extra_conditioning_channels != 1:
        raise ValueError("Inpainting controlnet must have extra_conditioning_channels=1.")
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            StableDiffusion3ControlNetInpaintingPipeline.save_lora_weights(
                output_dir,
    ...
    def load_model_hook(models, input_dir):
        ...
        lora_state_dict = StableDiffusion3ControlNetInpaintingPipeline.lora_state_dict(input_dir)
    ...
    for epoch in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                image_latents = vae.encode(conditioning_pixel_values).latent_dist.sample()
                image_latents = (image_latents - vae.config.shift_factor) * vae.config.scaling_factor
                image_latents = image_latents.to(dtype=weight_dtype)

                mask = batch["conditioning_mask"].to(dtype=weight_dtype)
                mask = torch_nn_functional.interpolate(
                    mask, size=(image_latents.shape[-2], image_latents.shape[-1]), mode="nearest"
                )
                mask = 1 - mask
