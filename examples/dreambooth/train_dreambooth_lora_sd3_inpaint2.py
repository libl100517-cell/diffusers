...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="sd3-dreambooth-inpaint",
...
class CrackInpaintDataset(Dataset):
    def __init__(self, args):
        ...
        if args.with_prior_preservation:
            self.class_data_root = Path(args.class_data_dir)
            self.class_data_root.mkdir(parents=True, exist_ok=True)
            self.class_images_path = list(self.class_data_root.iterdir())
            if args.num_class_images is not None:
                self.num_class_images = min(len(self.class_images_path), args.num_class_images)
            else:
        ...
        self.class_mask = torch.ones(1, args.resolution, args.resolution)
    ...
    def __getitem__(self, index):
        ...
        example = {
            ...
            "masked_instance_images": masked_pixel_values,
            "instance_mask": train_mask_tensor,
        ...
        if self.class_data_root:
            ...
            example["class_masked_images"] = class_image
            example["class_mask"] = self.class_mask.clone()
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    masked_pixel_values = [example["masked_instance_images"] for example in examples]
    mask = [example["instance_mask"] for example in examples]
    prompts = [example["instance_prompt"] for example in examples]

    if with_prior_preservation:
        pixel_values += [example["class_images"] for example in examples]
        masked_pixel_values += [example["class_masked_images"] for example in examples]
        mask += [example["class_mask"] for example in examples]
        prompts += [example["class_prompt"] for example in examples]

    pixel_values = torch.stack(pixel_values).to(memory_format=torch.contiguous_format).float()
    masked_pixel_values = torch.stack(masked_pixel_values).to(memory_format=torch.contiguous_format).float()
    mask = torch.stack(mask).to(memory_format=torch.contiguous_format).float()
...
def main(args):
    ...
    if args.cache_latents:
        latents_cache = []
        masked_latents_cache = []
        for batch in tqdm(train_dataloader, desc="Caching latents"):
            with torch.no_grad():
                batch["pixel_values"] = batch["pixel_values"].to(
                    accelerator.device, non_blocking=True, dtype=weight_dtype
                )
                batch["masked_pixel_values"] = batch["masked_pixel_values"].to(
                    accelerator.device, non_blocking=True, dtype=weight_dtype
                )
                latents_cache.append(vae.encode(batch["pixel_values"]).latent_dist)
                masked_latents_cache.append(vae.encode(batch["masked_pixel_values"]).latent_dist)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if args.cache_latents:
                    ...
                    masked_latents = masked_latents_cache[step].sample()
                else:
                ...
                masked_latents = (masked_latents - vae_config_shift_factor) * vae_config_scaling_factor
